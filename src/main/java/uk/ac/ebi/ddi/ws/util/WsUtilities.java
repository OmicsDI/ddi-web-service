...
public class WsUtilities {
    ...
    public static List<DatasetSummary> transformDatasetSummary(QueryResult queryResult, String domain,
                                                               Map<Tuple<String, String>, Integer> visitMap) {
        ...
        if (queryResult != null && queryResult.getEntries() != null && queryResult.getEntries().length > 0) {
            ...
            for (Entry entry: queryResult.getEntries()) {
                ...
                if (publicationDates != null && publicationDates.length > 0) {
                    datasetSummary.setPublicationDate(publicationDates[0]);
                }
                if (omicsType != null && omicsType.length > 0) {
                    datasetSummary.setOmicsType(Arrays.asList(omicsType));
                }
    ...
    public static List<DatasetSummary> transformSimilarDatasetSummary(QueryResult queryResult,
                                                                      String domain, Map<String, String> scores) {
        ...
        if (datasetDetail != null && datasetDetail.getKeywords() != null && datasetDetail.getKeywords().length > 0) {
            for (String oldkeyword: datasetDetail.getKeywords()) {
                if (oldkeyword.split(";").length > 1) {
                    ...
                    newKeywords.addAll(Arrays.asList(keywordArr));
    ...
    public static DatasetDetail mapSimilarsToDatasetDetails(DatasetDetail dataset, DatasetSimilars similars) {
        ...
        if (similars != null && similars.getSimilars() != null) {
            for (SimilarDataset similar: similars.getSimilars()) {
                if (similar.getSimilarDataset() != null) {
                    dataset.addSimilar(similar.getSimilarDataset().getAccession(),
                            similar.getSimilarDataset().getDatabase(), similar.getRelationType());
                }
            }
        }
        return dataset;
